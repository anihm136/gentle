...
void usage() { fprintf(stderr, "usage: k3 [nnet_dir hclg_path]\n"); }
...
int main(int argc, char *argv[]) {
  ...
  nnet3::DecodableNnetSimpleLoopedInfo de_nnet_simple_looped_info(
      nnet_simple_looped_opts, &am_nnet);
  ...
  SingleUtteranceNnet3Decoder decoder(
      nnet3_decoding_config, trans_model, de_nnet_simple_looped_info,
  ...
  while (true) {
    ...
    if (strcmp(cmd, "stop\n") == 0) {
      ...
      new (&decoder) SingleUtteranceNnet3Decoder(
          nnet3_decoding_config, trans_model, de_nnet_simple_looped_info,
          // am_nnet,
          *decode_fst, &feature_pipeline);
      ...
      for (int i = 0; i < words.size(); i++) {
        ...
        fprintf(stdout, "word: %s / start: %f / duration: %f\n",
                word_syms->Find(words[i]).c_str(), times[i] * frame_shift,
